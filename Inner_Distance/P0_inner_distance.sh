...
do
  ...
  grep "100M" inner_distance_P0_${ITEM}.txt | grep "=" |awk '{ if( $6>0 ){print $6-200} }' > ${Output}/P0_id_${ITEM}.txt
  #only filter 100% match reads and match on the same transcripts to calculate the inner distance
  #minus 200 due to the length of the reads is 100 base pa
  #only use the + PLENGTH
  echo "finish P0_${ITEM} the inner distance !"
done

##test scripts
head -50 inner_distance_P0_01.txt | grep "100M" | grep "=" |awk '{ if( $6>0 ){print $6-200} }'
